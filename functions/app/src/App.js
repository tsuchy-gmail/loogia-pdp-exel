...
let organizationList = localStorage.organizationList ? JSON.parse(localStorage.organizationList) : 
  [
    {
      name: '土本運輸',
      ApiKey: API_key,
...
let numberOfDepot = localStorage.numberOfDepot ? JSON.parse(localStorage.numberOfDepot) : 0;

let numberOfOrganization = localStorage.numberOfOrganization ? JSON.parse(localStorage.numberOfOrganization) : 0;

const depotList = localStorage.depotList ? JSON.parse(localStorage.depotList) : 
 [
   {
      name: "東京駅",
      lat: 35.681254,
...
function Depot() {
  ...
  const deleteStyle = {outline: 0, width: '120px', fontSize: '13px', left: '410px'}
  ...
  return (
    ...
    <div className={classes.root}>
      ...
      <Button style={registrationStyle}  color="primary" onClick={handleClickOpenRegister}>
        ＞ デポを登録
      </Button>
      <Button style={deleteStyle}  color="secondary" onClick={handleClickOpenDelete}>
        <DeleteSweepTwoToneIcon style={{marginRight: '10px' }} /> デポを削除
      </Button>

      </div>
    </div>
        <FormControl className={classes.root}>
        <InputLabel id="demo-simple-select-label" style={{width: '200px'}}>デポを選択</InputLabel>
        <Select
        ...
        >
          {depotList.map((depot, index) => (<MenuItem value={index}>{depot.name}</MenuItem>))}
        </Select>
        </FormControl>
      <Dialog open={openRegister} onClose={handleCloseRegister} aria-labelledby="form-dialog-title">
        ...
        <DialogActions>
          <Button style={noOutline} onClick={handleCloseRegister} color="primary">
            Cancel
          </Button>
      ...
      <Dialog open={openDelete} onClose={handleCloseDelete} aria-labelledby="form-dialog-title">
        ...
        <Select
          ...
          style={{width: '400px'}}
        >
          {depotList.map((depot, index) => (<MenuItem value={index} onClick={() => handleClickSelectInDelete(index)}>{depot.name}</MenuItem>))}
        </Select>
...
function Carrier() {
  ...
  const createForm = (i) => {
    ...
    const htmlForm = (
      <form className={classes.root} style={{marginTop: '15px'}} noValidate autoComplete="off">
        <FormControl>
  ...
  return (
    <div className={classes.root}>
      <div style={{ width: "800px", marginTop: "30px" }}>
        ...
        <span style={{color: "#3f50b5", fontSize:'14px', marginLeft: '8px'}}>Excelファイルを選択</span>
        <input
          type="file"
          id="file"
        ...
        <label htmlFor="file">
          <Button
            variant='outlined'
            style={{ outline: 0, width: "727px", marginTop: '20px', marginLeft: '4px'}}
            component="span"
          >
  ...
  return (
    <FormControl className={classes.root}>
        ...
        >
          ...
          <MenuItem value={true}>制限する(全てのspotに10000秒のuTurnCostが振られています)</MenuItem>
        </Select>
    </FormControl>
  )
}
  ...
  return (
    <FormControl className={classes.root}>
        ...
        <Select
          ...
          style={{width: '730px'}}
        >
          <MenuItem value={false}>デポへの帰り道は最適化に含めない (default)</MenuItem>
          <MenuItem value={true}>デポへの帰り道まで最適化に含める</MenuItem>
        </Select>
    </FormControl>
  )
}
...
function KeepStraight(){
  ...
  return (
    <FormControl className={classes.root}>
        <InputLabel style={{width: '100px'}} id="demo-simple-select-label">直進維持度</InputLabel>
        <Select
...
function DirectionRestriction(){
  ...
  return (
    <div>
        <FormControl className={classes.root}>
        <InputLabel id="demo-simple-select-label" style={{width: '200px'}}>右左折どちらか抑制</InputLabel>
        <Select
          defaultValue={options.turnDirectionRestriction ? options.turnDirectionRestriction.direction : 0}
          labelId="demo-simple-select-label"
          id="demo-simple-select"
        ...
        <Select
          defaultValue={options.turnDirectionRestriction ? options.turnDirectionRestriction.intensity : 0}
          labelId="demo-simple-select-label"
          id="demo-simple-select"
          onChange={handleChangeIntensity}
          style={{width: '357px'}}
...
function CalculationTime(){
  const classes = useStyles();
  let initialValue = '';
  if(localStorage.options && JSON.parse(localStorage.options).calculationTime){
...
function Organization(){
  ...
  const noOutline = {outline: 0}
  const registrationStyle = {outline: 0, width: '120px', fontSize: '13px'}
  const deleteStyle = {outline: 0, width: '120px', fontSize: '13px', left: '362px'}
  ...
  return (
    ...
    <div className={classes.root}>
      ...
      <Button style={deleteStyle}  color="secondary" onClick={handleClickOpenDelete}>
        <DeleteSweepTwoToneIcon style={{marginRight: '10px' }} /> 組織を削除
      </Button>

      </div>
    </div>
        <FormControl className={classes.root}>
        <InputLabel id="demo-simple-select-label" style={{width: '200px'}}>組織を選択</InputLabel>
        <Select
          value={selectedNumber}
          labelId="demo-simple-select-label"
          id="demo-simple-select"
          ...
          onChange={handleChangeSelect}
        >
          {organizationList.map((organization, index) => (<MenuItem value={index}>{organization.name}</MenuItem>))}
        </Select>
        </FormControl>
      <Dialog open={openRegister} onClose={handleCloseRegister} aria-labelledby="form-dialog-title">
      ...
      <Dialog open={openDelete} onClose={handleCloseDelete} aria-labelledby="form-dialog-title">
        ...
        <Select
          ...
          style={{width: '400px'}}
        >
          {organizationList.map((organization, index) => (<MenuItem value={index} onClick={() => handleClickSelectInDelete(index)}>{organization.name}</MenuItem>))}
        </Select>
        ...
        onSubmit={(e) => {
          ...
          if (file) {
            if (file.name.substr(-5, 5) === ".xlsx") {
              XLSX(file, projectName, depotList[numberOfDepot], carriersInfo, options, organizationList[numberOfOrganization]);
            } else {
